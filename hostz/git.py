...
class GitMixIn:
    def git_pull(self, project_path=None):
        ...
        self.run(f'cd {project_path} && git pull')

    def git_checkout(self, branch, project_path):
        """调用git checkout 更新分支"""
        self.run(f'cd {project_path} && git stash && git checkout {branch} && git pull')

    def get_commit_id(self, short=True):
        """获取当前commit_id"""
        cmd = 'git rev-parse --short HEAD' if short is True else 'git rev-parse HEAD'
        return self.run(f'cd {self.workspace}/chainmaker-go && {cmd}')

    def get_branch(self):
        """获取当前分支"""
        return self.run("git branch | sed -n '/\* /s///p'")
    ...
    def clone_or_update(self, repo, branch='master'):
        ...
        if not self.exists(f'{self.workspace}/{project_name}'):
            ...
            result = self.run(f'cd {self.workspace} && git clone --recursive -b {branch} {repo}')
